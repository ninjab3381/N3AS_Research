...
C..........ACCUMULATE.
       IF ((t9.le.t9f).or.(dt.lt.abs(cl/dlt9dt))
     a     .or.(ip.eq.inc))  CALL accum                 
     
C     The above refer to Low temp, Small dt, Enough iterations.
...
C..........ADJUST TIME STEP.
     ...
     a             *(1.+(alog10(y(i))/alog10(ytmin))**2)  !(Ref 2).
             IF (dtl.lt.dtmin) dtmin = dtl        
              !Find smallest time step.
           END IF
         END DO
         IF (dtmin.gt.1.5*dt) dtmin = 1.5*dt      
...
C..........STORE AND INCREMENT VALUES (Ref 3).
       DO i = 1,mvar
         ...
         dvdt0(i) = dvdt(i)
         v(i)     = v0(i) + dvdt0(i)*dt
         IF ((i.ge.4).and.(v(i).lt.ytmin)) v(i) = ytmin  
...
C         (d(t9)/dt)*(dt/t9) < ct
C         Wagoner, R.V. 1969, Ap J. Suppl. No. 162, 18, page 293,
C         equation C6.
C     2)  Constraint on dt from
C         dtl < y/(dy/dt)*cy*(1+(log(y)/log(ytmin))**2)
C         Wagoner, R.V. 1969, page 293, equation C7 but with log 
C         term squared.
...
C----------PARAMETERS.
       ...
       PARAMETER (nnuc=26)         !Number of nuclides in calculation.
       PARAMETER (const1=0.09615)  !Relation between time and temp.        
...
C----------COMMON AREAS.
       ...
       COMMON /bessl/ bl1,bl2,bl3,bl4,bl5       !Eval function bl(z).
       COMMON /bessl/ bm1,bm2,bm3,bm4,bm5      !Eval function bm(z).
       COMMON /bessl/ bn1,bn2,bn3,bn4,bn5       !Eval function bn(z).
       
C//////Changed the name of the above list fom 'bessel' to 'bessl', as
C      'bessel' is the name of a subroutine.
...
C----------REACTION RATES.
       REAL    f(nrec)        !Forward reaction rate coefficients. (array)
       REAL    r(nrec)        !Reverse reaction rate coefficients. (array)
...
C30--------COMPUTE INITIAL ABUNDANCES FOR NEUTRON AND PROTON-----
       ...
       IF ((15.011/t9+xi(1)).gt.58.) THEN      
       !Overabundance of antineutrinos.
         y(1) = 1.e-25              !Very little of neutrons.  ???
         y(2) = 1.                  !Essentially all protons.  ???
       ELSE
         IF ((15.011/t9+xi(1)).lt.-58.) THEN   
         !Overabundance of neutrinos.
           y(1) = 1.                !Essentially all neutrons. ???    
           y(2) = 1.e-25            !Very little of protons.   ???
         ELSE
           y(1) = 1./(ex(15.011/t9+xi(1))+1.)  
           !Initial n abundance (Ref 3).
           y(2) = 1./(ex(-15.011/t9-xi(1))+1.) 
           !Initial p abundance (Ref 3).
         END IF
       END IF
...
C40--------FIND RATIO OF BARYON DENSITY TO TEMPERATURE CUBED---------
       ...
       phie   = hv*(1.784e-5*y(2))/
     a  (.5*z**3*(bl1-2.*bl2+3.*bl3-4.*bl4+5.*bl5)) 
       
       !Chemical potential of electron (Ref 5).
                 
       rhob0  = hv*t9**3           !Baryon density.
       IF ((xi(1).eq.0.).and.(xi(2).eq.0.).and.(xi(3).eq.0)) THEN
       !Nondegen.
         rhone0 = 7.366*t9**4      
...
C//////Changed above list name from 'therm' to 'thermo', 
C      as 'therm' is a subroutine.
...
110    CONTINUE
         CALL rate3                 
         !Forward rate for reactions with A < 19.
120    CONTINUE
         CALL rate2                 
         !Forward rate for reactions with A < 10.
C..........SOLVE COUPLED DIFFERENTIAL EQUATIONS.
       CALL sol(loop)
       IF (mbad.gt.0) RETURN        
       !Abort in case matrix not invertible.

C20--------COMPUTE DERIVATIVES FOR TEMPERATURE, hv, 
...
C..........ACCUMULATE TO GET SUM.
       DO i = 1,isize
         ...
         summdy = summdy + dm(i)*dydt(i)  
         !Sum of (mass excess)*(abundance flow).
         sumzdy = sumzdy + zm(i)*dydt(i)  
         !Sum of (charge)*(abundance flow).
       END DO
C..........CHANGES IN TEMPERATURE, hv, AND CHEMICAL POTENTIAL.
     ...
     a           sumy)/(thm(1) + thm(3) + thm(4) + thm(7) + thm(9)*bar
     a           + thm(6)*(dphdln + dphdzy*sumzdy/(3.*hubcst)))   !(Ref 1).
       dt9    = (3.*hubcst)/dlndt9
       dlt9dt = dt9/t9
...
C----------COMMON AREAS.
       ...
       COMMON /flags/  ltime,is,ip,it,mbad     !Flags,counters.
       COMMON /outdat/ xout,thmout,t9out,tout,dtout,etaout,hubout  
       					       !Output data.
       COMMON /runopt/ irun,isize,jsize        !Run options.
...
C..........SUM UP ABUNDANCES OF HEAVY NUCLIDES.
     ...
     a              +xout(it,14)+xout(it,15)+xout(it,16)+xout(it,17)
     a              +xout(it,18)+xout(it,19)+xout(it,20)+xout(it,21)
     a              +xout(it,22)+xout(it,23)+xout(it,24)+xout(it,25)
     a              +xout(it,26)   !Li8 to O16.
...
C----------COMMON AREAS.
       ...
       COMMON /thermo/  thm,hubcst             !Dynamic variables.
       COMMON /endens/ rhone0,rhob0,rhob,rnb   !Energy densities.
       COMMON /bessl/ bl1,bl2,bl3,bl4,bl5      !Eval of function bl(z).
       COMMON /bessl/ bm1,bm2,bm3,bm4,bm5      !Eval of function bm(z).
       COMMON /bessl/ bn1,bn2,bn3,bn4,bn5      !Eval of function bn(z).        
       COMMON /nupar/  t9mev,tnmev,tnu,cnorm,nu,rhonu 
...
C20--------COMPUTE THERMODYNAMIC VARIABLES--------------------
       ...
       thm(3)  = thm(1)/3.                                !(Ref 3).
       thm(4)  = 3206.*(bm1*cosh1 - bm2*cosh2 + bm3*cosh3 
     a            - bm4*cosh4 + bm5*cosh5)                !(Ref 4).
       thm(5)  = 3206.*(z/t9)*(bn1*cosh1 - 2.*bn2*cosh2   
     a      + 3.*bn3*cosh3 - 4.*bn4*cosh4 + 5.*bn5*cosh5) !(Ref 5).
       thm(6)  = 3206.*(bm1*sinh1 - 2.*bm2*sinh2 + 3.*bm3*sinh3  
     a      - 4.*bm4*sinh4 + 5.*bm5*sinh5)                !(Ref 6).
       thm(7)  = 3206.*(bl1*cosh1/z - bl2*cosh2/(2.*z)     
     a           + bl3*cosh3/(3.*z) - bl4*cosh4/(4.*z)
     a           + bl5*cosh5/(5.*z))                      !(Ref 7).
       IF ((xi(1).eq.0.).and.(xi(2).eq.0.).and.(xi(3).eq.0)) THEN  
       !Nondegen.
         thm(8) = xnu*rhone0*(rnb**(4./3.))                     
          !(Ref 8).
       ELSE                         
       !Include effects of neutrino degeneracy.
         thm(8) = 0.
         DO nu = 1,xnu              
         !For every neutrino family.
           CALL nudens              
           !Compute neutrino energy density.
           thm(8) = thm(8) + 12.79264*rhonu 
           !Have 12.79264 from units change.
         END DO
       END IF
       thm(9)  = rhob0*rnb                                       
       !(Ref 9).
       ...
       !(Ref 10).
       thm(11) = -(z**3/t9)*(sinh1*(3.*bl1-z*bm1)-sinh2*(3.*bl2  
     a     -2.*z*bm2) + sinh3*(3.*bl3-3.*z*bm3) - sinh4
     a     *(3.*bl4-4.*z*bm4) + sinh5*(3.*bl5-5.*z*bm5))   !(Ref 11).
       thm(12) = z**3*(cosh1*bl1- 2.*cosh2*bl2                   
     a     + 3.*cosh3*bl3 - 4.*cosh4*bl4 + 5.*cosh5*bl5)   !(Ref 12). 
       IF (thm(12).ne.0.) thm(12) = 1./thm(12)
       thm(13) = 1.000 + 0.565/z1 - 6.382/z2 + 11.108/z3         
     a            + 36.492/z4 + 27.512/z5                  !(Ref 13).
       thm(14) = (5.252/z1 - 16.229/z2 + 18.059/z3 + 34.181/z4   
     a             + 27.617/z5)*ex(-q*z)


       IF (loop.eq.1) THEN
            WRITE(11,*) thm(14), t9mev
            WRITE(10,*) thm(13), t9mev
       END IF
...
C----------COMMON AREAS.
       COMMON /bessl/ bl1,bl2,bl3,bl4,bl5  !Evaluation of function bl(z).
       COMMON /bessl/ bm1,bm2,bm3,bm4,bm5  !Evaluation of function bm(z).
       COMMON /bessl/ bn1,bn2,bn3,bn4,bn5  !Evaluation of function bn(z).
...
C----------EVALUATION OF FUNCTIONS bl,bm,bn.
       REAL    bl1,bl2,bl3,bl4,bl5 
       !Single variables equivalenced to array blz.
       REAL    bm1,bm2,bm3,bm4,bm5  
       !Single variables equivalenced to array bmz.
       REAL    bn1,bn2,bn3,bn4,bn5  
       !Single variables equivalenced to array bnz.
...
C----------LOCAL VARIABLES.
       ...
       REAL    bmz(5)               
       !Array containing values from function bm.
       REAL    bnz(5)               
       !Array containing values from function bn.
       REAL    z                    
...
C----------EXPANSION COEFFICIENTS.
       DATA ci0 / 1.,
     a            3.5156229,      3.0899424,      1.2067492,
     a            0.2659732,      0.0360768,      0.0045813/
       DATA ci1 / 0.5,
     a            0.87890594,     0.51498869,     0.15084934,
     a            0.02658733,     0.00301532,     0.00032411/
       DATA ck0 /-0.57721566,
     a            0.42278420,     0.23069756,     0.03488590,
     a            0.00262698,     0.00010750,     0.00000740/
       DATA ck1 / 1.,
     a            0.15443144,    -0.67278579,    -0.18156897,
     a           -0.01919402,    -0.00110404,    -0.00004686/
       DATA c0  / 1.25331414,
     a           -0.07832358,     0.02189568,    -0.01062446,
     a            0.00587872,    -0.00251540,     0.00053208/
       DATA c1  / 1.25331414,
     a            0.23498619,    -0.03655620,     0.01504268,
     a           -0.00780353,     0.00325614,    -0.00068245/
...
C----------COMMON AREAS.
       COMMON /modpr/  g,tau,xnu,c(3),cosmo,xi        
       !Model parameters.
       COMMON /nupar/  t9mev,tnmev,tnu,cnorm,nu,rhonu 
       !Integration parameters.
...
C//////Not sure what an EXTERNAL data type is (also, see below).       
...
C========================IDENTIFICATION DIVISION=================
       ...
       REAL FUNCTION eval(x)             !A function subprogram.
       					 
C//////The argument '(x)' was added to 'eval'.	
...
C//////func5 and func6 are of type EXTERNAL in NUDENS. 
...
C10--------1ST PART OF INTEGRAL FOR n->p RATE---------------------
       ...
       ENTRY func1(x)               !First ENTRY point into FUNCTION eval
       IF (x.le.0.) THEN
         func1 = 0.
       ELSE
...
C----------ABSCISSAS AND WEIGHT FACTORS.
     ...
     a         .23861918608320, .66120938646627, .93246951420315/
       DATA w/.17132449237917,.36076157304814,.46791393457269,
     a         .46791393457269,.36076157304814,.17132449237917/
...
C-----------COMMON AREAS.
       ...
       COMMON /thermo/  thm(14),hubcst        !Dynamic variables.
       COMMON /endens/ rhone0,rhob0,rhob,rnb !Energy densities.
...
207        CONTINUE                 !3-0-0-1 configuration.
             r(n) = rev(n)*1.e+20*t932*t932*ex(-q9(n)/t9)*f(n)  
             !(Ref 4).
             f(n) = rhob*rhob*f(n)
             ci = y(i)*y(i)*f(n)/6.
             cj = 0.
...
209        CONTINUE                 !1-1-1-2 configuration.
             f(n) = rhob*f(n)
             r(n) = rev(n)*1.e-10*t9m32*rhob*ex(-q9(n)/t9)*f(n)  
             !(Ref 5).
             ci = y(j)*f(n)/2.
             cj = y(i)*f(n)/2.
             ck = y(l)*y(l)*r(n)/6.
...
C----------COMMON AREAS.
       COMMON /compr/  cy,ct,t9i,t9f,ytmin,inc   !Comp parameters.
