...
C     a flexible module which allows user
C     to manipulate physical quantities
C     of interest at certain key points during the computer run.
C     Included within this subroutine is 
C     a roster of all global variables
C     and their respective COMMON areas.
C     Current instructions accumulate abundances 
C     of deuterium, helium-3,
C     helium-4, and lithium-7 for eventual plotting, 
C     taking into account
C     the contribution of beryllium-7 to lithium-7 
C     and tritium to helium-3.
...
C----------COMMON AREAS.
       ...
       COMMON /recpr/  iform,ii,jj,kk,ll,rev,q9       
       !Reaction parameter names.
       COMMON /rates/  f,r                   
       !Reaction rates.
       COMMON /evolp1/ t9,hv,phie,y                   
       !Evolution parameters.
       COMMON /evolp2/ dt9,dhv,dphie,dydt             
       !Evolution parameters.
       COMMON /evolp3/ t90,hv0,phie0,y0               
       !Evolution parameters.
       COMMON /compr0/ cy0,ct0,t9i0,t9f0,ytmin0,inc0  
       !Default comp parameters.
       COMMON /compr/  cy,ct,t9i,t9f,ytmin,inc        
       !Computation parameters.
       COMMON /modpr0/ c0,cosmo0,xi0                  
       !Default model parameters.
       COMMON /modpr/  g,tau,xnu,c,cosmo,xi           
       !Model parameters.
       COMMON /varpr0/ dt0,eta0                       
       !Default variationl params.
       COMMON /varpr/  dt1,eta1                       
       !Variational parameters.
       COMMON /time/   t,dt,dlt9dt                    
       !Time variables.
       COMMON /thermo/  thm,hubcst                     
       !Dynamic variables.
       COMMON /endens/ rhone0,rhob0,rhob,rnb          
       !Energy densities.
       COMMON /lncoef/ a,b,yx                         
       !Linear eqn coefficients.
       COMMON /nucdat/ am,zm,dm                       
       !Nuclide data.
       COMMON /bessl/ bl1,bl2,bl3,bl4,bl5          
      !Eval function bl(z).
       COMMON /bessl/ bm1,bm2,bm3,bm4,bm5            
       !Eval function bm(z).
       COMMON /bessl/ bn1,bn2,bn3,bn4,bn5           
        !Eval function bn(z).
       COMMON /kays/   bk0,bk1,bk2,bk3,bk4            
       !Coefficients K.
       COMMON /flags/  ltime,is,ip,it,mbad            
       !Flags,counters.
       COMMON /checki/  itime                          
       ...
       !Neutrino parameters.
       COMMON /runopt/ irun,isize,jsize               
       !Run options.
       COMMON /outopt/ nout,outfile                   
       !Output option.
...
C----------EARLY UNIVERSE MODEL PARAMETERS.
       ...
       REAL    xi(3)      !Neutrino degeneracy parameters.
                          ...
                          !x(3) is t neutrino degeneracy parameter.
...
C10--------OPEN FILE-----------------------------------------------
       ...
       IF (itime.eq.1) THEN         !Beginning of program.
         OPEN (unit=3, file='nucint.dat',  status='new')
       END IF
       
C//////This creates a file 'nucint.dat' which can not exist at compilation.      
C//////The program will open the new file when executing the above line.       
...
C20--------WRITE INTO FILE-----------------------------------------
       ...
       IF (itime.eq.8) THEN         !Right after a run.
         ...
         WRITE (3,200) etaout(it),xout(it,3),xout(it,5)
         WRITE (3,200) xout(it,6),xout(it,8)
...
C----------REFERENCES--------------------------------------------
C  1) D.A. Dicus, E.W. Kolb, A.M. Gleeson, E.C.G. Sudarshan,
C     V.L. Teplitz,
