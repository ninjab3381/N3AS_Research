...
! This program calculates sigma_v depending on given sigma or s_factor     
    ...
    ! Executable statements 
       ...
       dS_dE_2k = ((5.973*10.0**(-3.0)) + (5.449*10.0**(-6.0)*2.0*2000.0) -  & 
       (1.656*10.0**(-9.0)*3.0*2000.0**(2.0))) * 10.0**(-6.0)

       S_root_2k = ((0.2121 + 5.973*10.0**(-3.0)*2000.0) + (5.449*10.0**(-6.0)*2000.0**(2.0)) -  & 
       (1.656*10.0**(-9.0)*2000.0**(3.0))) * 10.0**(-6.0)
       ...
       do i= 1, size(T9)
         ...
         open(1, file = 'sigma_v_d(p,g)3He_gauss.dat', status = 'unknown')  
